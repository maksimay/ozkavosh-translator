...
for key, value in dict_syndLUL.items():                             # for each key value pair item:
            ...
            templist = np.unique(templist).tolist()                 # for each key, remove duplicates from templist!
...
for i in looprange:
    ...
    Current_Word = str(BigList[i])
    Current_Word = [character for character in str.lower(Current_Word) if character.isalnum()] # clean out [""] etc
    ...
    if English_Word in (item for sublist in dict_syndLUL.values() for item in sublist):
        ...
        print("jop da gibts schon ne übersetzung für :)")
    ...
    print("translated word is", Current_Word)
    ...
    for i in start_chars:
        ...
        ozk_syllables.append(ozk_audio_result)
